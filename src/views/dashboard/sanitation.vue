        ...
        <v-row>
          ...
          <v-col md="auto">
            <v-btn rounded color="primary" style="margin-top: -10px;" @click="addNewDuty = true">
              <v-icon>mdi-plus</v-icon>Add new sanitation record
            </v-btn>
...
export default {
  ...
  methods: {
    ...
    getRecord() {
      ...
      axios.get('http://admin-database.herokuapp.com/sanitation/getAll')
      .then(Response => {
    ...
    add() {
      ...
      axios.post('http://admin-database.herokuapp.com/sanitation/addNewSanitation/user/id', data, config)
